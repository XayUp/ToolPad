...
public class AutoPlayFunc {
    ...
    AutoPlayFunc(Activity context){
		...
		touch = false;
		lineInt = 0;
		autoplaySize = VariaveisStaticas.autoPlay.size();
		time = 0;
		padWaiting = 0;
		chain = "19";
		seekChange= false;
    ...
    */
	...
	protected void chainChanged(){
		if(!(""+VariaveisStaticas.chainId).equals(chain))
	...
	//	VariaveisStaticas.progressAutoplay.setProgress(getProgress());
	}
	...
	private void pausedEvents(final int ViewId, int color, float alpha, int request){
		context.runOnUiThread(new Runnable(){
			...
			public void run() {
				...
				switch(request){
					case REQUEST_CHAIN:
						((ImageView)touchInView.findViewById(R.id.press)).setAlpha(VariaveisStaticas.watermark);
	...
	private void autoplaEvents(final int ViewId, final int request, int TOUCH){
		if(paused.get()){
			...
			pausedEvents(ViewId, 0, VariaveisStaticas.padPressAlpha, request);
			touch = false;
			seekChange = false;
			if(chainChanged){
				chainChanged = false;
            ...
            new Runnable() {
              ...
              public void run() {
                ...
                autoplayLoop: for (lineInt = 0; lineInt < autoplaySize; lineInt++) {
                 /* if (!VariaveisStaticas.progressAutoplay.getStatePressed()) {
                    progressUpadate();
                  } */
                  line = VariaveisStaticas.autoPlay.get(lineInt);
                  if (running.get()) {
                    ...
                    if (!line.isEmpty()) {
                      ...
                      switch (padEvent) {
                        ...
                        default:
                          ...
                          if (!(((String) "" + VariaveisStaticas.chainId).equals(chain) && paused.get()))
                                    /*
                    ...
                    if(to_exit){
                        ...
                        */ 
                        VariaveisStaticas.autoPlay = null;
                    }
                context.runOnUiThread(
                    new Runnable() {
                      ...
                      public void run() {
                        ...
                        padWaiting = -1;
                        //((SeekBar) VariaveisStaticas.progressAutoplay).setVisibility(View.GONE);
                      }
                    });
              }
            });
