...
import com.xayup.toolpad.R;import com.xayup.toolpad.R;
		...
		case "o":
			...
			if (ye) { //verificar o codigo de cor
				if (Integer.parseInt(line.substring(line.indexOf("a") + 1)) > 127) {
					VariaveisStaticas.invalid_formats.add("(" + fileName + ") " + context.getString(R.string.invalid_led_color)
	...
	protected static Map<String /*chain+pad position+repeat*/, List<List<String>> /*led file readed*/> readKeyLEDs(
		...
		for (final File ledFile : files) {
			...
			if (checkKeyLEDname(fileName, lineIndex)) {
					...
					for (String line : keys) {
						...
						//	if (line.contains("*"))
							...
							if (checkkeyLED(context, line, ledFile.getName())) {
								...
								VariaveisStaticas.invalid_formats.add("(" + ledFile.getName() + ") "
				...
				VariaveisStaticas.invalid_formats
    ...
    public static Map<String, List<Integer>> readKeySoundsNew(Activity context, File keySound, String soundPath) {
		if (keySound.exists()) {
			try {
				...
				VariaveisStaticas.mSoundLoader = new SoundLoader(context);
				for (String line : keys) {
					int indextheSound = 3;
					int indexPad = 1;
				if((!line.replaceAll("\\s", "").isEmpty()))
					if (checkKeySound(line.replaceAll("\\s", ""))) {
						...
						VariaveisStaticas.mSoundLoader.loadSound(sound, sound_length, line.substring(0, indextheSound), ifToChain);
					} else {
						VariaveisStaticas.invalid_formats.add("(" + keySound.getName() + ")" + context.getString(R.string.invalid_sound) + " " + line);
					}
				}
				VariaveisStaticas.mSoundLoader.prepare();
				return sounds;
			} catch (IOException e) {
			}
		}
			...
			while (line != null) {
				if (!line.replaceAll(" ", "").isEmpty()) {
					...
					if (checkAutoPlayFormat(line)) {
						...
						VariaveisStaticas.invalid_formats.add(context.getString(R.string.invalid_autoplay) + " " + line);
					}
				}
