...
public class GetFilesTask extends AsyncTask<Void, Void, Boolean> {
	...
	//	VariaveisStaticas.end(context);
		bar.dismiss();
		super.onPostExecute(arg0);
	}

	public void getFiles() {
		for(File file : new File(VariaveisStaticas.getCurrentPath).listFiles(Readers.projectFiles)){
			switch (file.getName().toLowerCase()){
				...
				case "autoplay":
					...
					VariaveisStaticas.autoPlay = Readers.readautoPlay(context, file);
					VariaveisStaticas.autoPlayThread = new AutoPlayFunc(context);
					/*
				...
				case "keyled":
					...
					VariaveisStaticas.ledFiles = Readers.readKeyLEDs(context, file);
					break;
			}
		}
	}
