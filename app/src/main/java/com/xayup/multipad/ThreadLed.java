...
public class ThreadLed implements Runnable {
    ...
    private void showLed(final int padid, final int corCode, final boolean MC){
        context.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							int color = VariaveisStaticas.colorInt(corCode, VariaveisStaticas.custom_color_table, VariaveisStaticas.oldColors);
						//	byte NOTE = MidiStaticVars.NOTE_ON;
						//	if(corCode == 0) NOTE = MidiStaticVars.NOTE_OFF;
							if(VariaveisStaticas.glowEf && padid != 9){
									ImageView glowEF = context.findViewById(Integer.parseInt("100"+padid));
									if(color == 0){
										glowEF.setAlpha(0.0f);
									} else {
										if(MC){
											glowEF.setAlpha(VariaveisStaticas.glowChainIntensity);
										} else {
											glowEF.setAlpha(VariaveisStaticas.glowIntensity);
										}
	...
	@Override
	public void run() {
		if (VariaveisStaticas.ledFiles.get(cpled) != null) {
			long time = SystemClock.uptimeMillis();
			boolean nobreak = true;
			boolean delay = false;
            int indexLoop = 1;
            try {
                loop = Integer.parseInt(VariaveisStaticas.ledFiles.get(cpled).get(rpt).get(0));
                } catch (NumberFormatException n){
                    loop = 1;
                } catch (IndexOutOfBoundsException i){}
			String line;
            looper: while(true){
            for (int i = 1; i < VariaveisStaticas.ledFiles.get(cpled).get(rpt).size(); i++) {
                line = VariaveisStaticas.ledFiles.get(cpled).get(rpt).get(i);
				if (!isRunning() || VariaveisStaticas.stopAll) {
                    if(loop == 0)
                        offCurrentLedLoop(VariaveisStaticas.ledFiles.get(cpled).get(rpt));
                    else
				    	XayUpFunctions.clearLeds(context, root);
					break looper;
				}
				//	String line = VariaveisStaticas.ledFiles.get(cpled).get(rpt).get(i);
				delay = false;
				int padId = 0;
				int corcode = 0;
				boolean mc = false;
				...
				//	VariaveisStaticas.ledOcuped.add(padId);
				...
				while ((SystemClock.uptimeMillis() < time) && (!VariaveisStaticas.stopAll) && isRunning()) {
				}
